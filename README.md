...
- [钉钉平台准备工作指南](docs/dingtalk-setup-guide.md)：在钉钉开放平台后台需要完成的准备工作（创建应用、机器人、权限、AI 卡片模板等）。
- [部署指南](DEPLOYMENT.md)：在真实环境中部署并长期运行本服务（环境要求、构建启动、进程守护、运维排查）。
...
- `streaming.templateId`：AI Card 完整 `cardTemplateId`；仅在 `streaming.mode` 为 `ai-card` 时必填。模板需包含 `streaming.contentKey` 指定的内容变量，默认变量名为 `content`。
